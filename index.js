...
app.post("/sure20oddspay", (req, res) => {
  ...
  paystack.transaction
    .initialize({
      ...
      amount: kobo_amount,
      phone: phone_number,
      callback_url: "http://localhost:8000/sure20odds",
    })
    .then(function (body) {
    ...
    .catch(function (error) {
      res.redirect("/sure20oddspay");
    });
});

app.post("/fiveoddspay", (req, res) => {
  const { email, amount, phone_number } = req.body;
  if (!email || !phone_number) res.redirect("/fiveoddspay");
  let kobo_amount = amount * 100;
  console.log("dog");
  paystack.transaction
    .initialize({
      ...
      amount: kobo_amount,
      phone: phone_number,
      callback_url: "http://localhost:8000/fiveodds",
    })
    .then(function (body) {
      //extract the reference
      let reference = body.data.reference;
...
app.get("/sure20odds", async (req, res) => {
  console.log("dog");
  const user_id = req.session.user_id;
  let { reference } = req.query;

  if (user_id) {
    const image = await Image.find({ type: "sure20odds" });
    const info = {
      user_id,
      image,
    };
    res.render("sure20odds.ejs", { info });
    return;
  }
...
app.post("/fiveodds", upload.array("file"), async (req, res) => {
  ...
  if (newImage.image.length === 0) return res.redirect("/home");
  await newImage.save();
...
app.post("/sure20odds", upload.array("file"), async (req, res) => {
  const newImage = new Image();
  newImage.type = "sure20odds";
  newImage.image = req.files.map((file) => ({
    url: file.path,
    filename: file.filename,
  }));
...
app.get("/sure20oddspay", (req, res) => {
  res.render("payment/sure20oddspay.ejs");
});
